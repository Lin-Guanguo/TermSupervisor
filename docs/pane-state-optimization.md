...
- Display: DONE/FAILED -> IDLE is delayed by 5s; notifications suppressed for short tasks (<3s) or when focused. Only content hash is retained; content is lost on restart.
- Persistence: pane/persistence v2 with checksum exists but is not wired into runtime.
...
- Persistence comfort: on restart, restore last visible state; if resuming LONG_RUNNING, mark as resumed and keep elapsed when possible.
...
### Phase 3 (should ship): Persistence wiring
Scope:
- Wire load/save in runtime/bootstrap; periodic save via Timer; save on shutdown best-effort.
- Generation bump immediately after load; checksum/version guard; tolerate missing/invalid snapshot.
- Persist last tick timestamp to avoid retriggering LONG_RUNNING on restart.
Done checklist:
- Tests for load + generation bump and graceful handling of invalid/missing snapshot.
- Config defaults for persist path/version and save cadence; telemetry counters for save/load success/failure.
- Logs show path/version and fallback behavior; no double LONG_RUNNING on first tick after restart.

### Phase 4 (optional/flagged): Content heuristics
Scope:
...
- Persistence: runtime/bootstrap.py (load/save wiring), config.py (path/version/cadence), telemetry.py (save/load counters).
...
- Tests: tests/test_pane_state_machine.py, tests/test_state_manager.py, tests/test_hook_manager.py for WAITING fallbacks, drop policy, stickiness, persistence; metric assertions.
...
- Persistence path/version and save cadence (e.g., 30-60s); whether persistence is opt-in via config.
...
- Success metrics: non-content drop rate <1%; WAITING timeouts <0.5% of waits; no transition latency regression; persistence load success >99% (counter); zero “double transition” logs in timer overlap tests.
- Testing matrix: fake timers for WAITING fallback and auto-clear; queue-under-pressure ensures command_end never drops; same-source RUNNING ignored in LONG_RUNNING; WAITING resume via hash heartbeat; persistence load + generation bump; WS emits/consumes content.update without dupes; heuristic start/idle/end guarded by flag.
- Observability/logging: rate-limit drop metrics/logs; standard prefixes [PaneSM]/[StateMgr]/[Pane]/[Heuristic]; include pane_id + generation in logs; counters for wait_fallback (content vs timeout), auto_clear, save/load success/failure, heuristic start/idle/drop.
...
- Single-responsibility modules: queue policy and thresholds live in pane/manager.py + pane/queue.py; heuristics isolated in analysis/heuristics.py (flagged); persistence wiring confined to runtime/bootstrap.py. Do not mix heuristic logic into state_machine/transitions.
...
- Persistence load may be slow/stale; add timeouts and fall back to clean start on checksum/version mismatch or excessive staleness.
